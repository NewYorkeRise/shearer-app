...
function App() {
  ...
  return <div className="app-shell">
    <aside className={`sidebar ${mobileNavOpen ? 'sidebar-open' : ''}`}>
      ...
      <div className="sidebar-bottom"><div className="google-card"><div className="google-card-icon">G</div><div><strong>{t('googleReady')}</strong><span>{t('googleBody')}</span></div></div><button className="settings-link" onClick={() => selectNav('profile')}><Settings2 size={17} /> {t('settings')}</button><button className="account-row account-button" onClick={() => setIsLoginOpen(true)}><div className="avatar">{currentUser.initials}</div><div className="account-copy"><strong>{currentUser.name}</strong><span>{currentUser.organization[language]}</span></div><MoreHorizontal size={17} className="muted-icon" /></button></div>
    </aside>

    <main className="main-content">
      <header className="topbar"><button className="icon-button menu-button" onClick={() => setMobileNavOpen(true)} aria-label={t('openMenu')}><Menu size={20} /></button><div className="topbar-context"><span className="context-kicker">{t('eyebrow')}</span><span className="context-title">{activeView === 'home' ? t('activeSeason') : navItems.find((item) => item.id === activeView)?.label}</span></div><div className="topbar-actions"><button className="control-button language-button" onClick={() => setLanguage(language === 'en' ? 'mn' : 'en')} title={t('switchLanguage')}><Globe2 size={16} /><span>{language === 'en' ? 'MN' : 'EN'}</span></button><button className="control-button theme-button" onClick={() => setTheme(theme === 'light' ? 'dark' : 'light')} title={t('switchTheme')}>{theme === 'light' ? <Moon size={16} /> : <Sun size={16} />}</button><button className="topbar-user login-trigger" onClick={() => setIsLoginOpen(true)} title={t('loginTitle')}><div className="avatar small">{currentUser.initials}</div><span>{currentUser.name} · {t('demoAccount')}</span></button></div></header>
      {activeView === 'home' && <HomeView t={t} language={language} role={role} jobs={jobs} coords={coords} mapUrl={mapUrl} onCreate={() => setIsModalOpen(true)} onJobs={() => selectNav('jobs')} onShare={shareLocation} onFindLocation={findLocation} showToast={showToast} />}
...
type HomeProps = { t: (key: string) => string; language: Language; role: Role; jobs: Job[]; coords: { lat: number; lng: number }; mapUrl: string; onCreate: () => void; onJobs: () => void; onShare: () => void; onFindLocation: () => void; showToast: (message: string) => void }
function HomeView({ t, language, role, jobs, coords, mapUrl, onCreate, onJobs, onShare, onFindLocation, showToast }: HomeProps) { return <div className="page-wrap"><section className="hero-grid"><div className="hero-copy"><div className="eyebrow"><Sparkles size={14} /> {t('fieldReady')}</div><h1>{t('heroTitle')}</h1><p>{t('heroBody')}</p><div className="hero-actions"><button className="primary-button" onClick={onCreate}><Plus size={17} /> {t('createJob')}</button><button className="quiet-button" onClick={onJobs}>{t('viewJobs')} <ArrowUpRight size={16} /></button></div><div className="hero-note"><ShieldCheck size={15} /> {t('trusted')}</div></div><div className="season-card"><div className="season-header"><div><span>{t('activeSeason')}</span><strong>{t('seasonTitle')}</strong></div><span className="status-pill"><span /> {t('live')}</span></div><div className="season-visual"><div className="season-orbit orbit-one" /><div className="season-orbit orbit-two" /><div className="season-center"><strong>09</strong><span>{t('sep')}</span></div></div><div className="season-footer"><div><strong>1,240</strong><span>{t('sheepThisSeason')}</span></div><div><strong>+38%</strong><span>{t('capacity')}</span></div></div></div></section><section className="metric-row"><MetricCard icon={ClipboardCheck} label={t('upcoming')} value="12" detail={t('weeklyChange')} tone="mint" /><MetricCard icon={CircleDollarSign} label={t('paidPayroll')} value="4.8M" detail={t('paidPayrollDetail')} tone="sand" /><MetricCard icon={Leaf} label={t('woolRecovered')} value="1.86t" detail={t('lastSeasonChange')} tone="blue" /><MetricCard icon={Clock3} label={t('avgResponse')} value="2.4h" detail={t('avgResponseDetail')} tone="lavender" /></section><section className="content-grid"><div className="panel activity-panel"><div className="panel-heading"><div><div className="eyebrow">{t('next')}</div><h2>{t('upcoming')}</h2></div><button className="link-button" onClick={onJobs}>{t('seeAll')} <ChevronRight size={15} /></button></div><div className="job-list">{jobs.slice(0, 3).map((job) => <JobRow key={job.id} job={job} language={language} t={t} onClick={onJobs} />)}</div><div className="panel-footer-note"><AlertCircle size={15} /> {t('paymentReleaseNote')}</div></div><div className="panel map-panel"><div className="panel-heading"><div><div className="eyebrow">{t('location')}</div><h2>{t('fieldMap')}</h2></div><button className="icon-button" onClick={onFindLocation} aria-label={t('location')}><Navigation size={17} /></button></div><div className="map-surface"><div className="map-road road-one" /><div className="map-road road-two" /><div className="map-road road-three" /><div className="map-river" /><div className="map-pin pin-one"><MapPin size={21} fill="currentColor" /></div><div className="map-pin pin-two"><MapPin size={21} fill="currentColor" /></div><div className="map-label label-one">{t('selenge')}</div><div className="map-label label-two">{t('tov')}</div><div className="map-coordinates">{coords.lat.toFixed(3)}, {coords.lng.toFixed(3)}</div></div><div className="map-footer"><div><strong>{t('selengeFieldSite')}</strong><span>{t('locationHint')}</span></div><div className="map-actions"><button className="small-action" onClick={onShare}><Share2 size={14} /> {t('shareLocation')}</button><a className="small-action ghost" href={mapUrl} target="_blank" rel="noreferrer"><ExternalLink size={14} /> {t('openMaps')}</a></div></div></div></section><section className="bottom-grid"><div className="statement-card"><div className="statement-icon"><Leaf size={19} /></div><div><div className="eyebrow">{t('impact')}</div><h2>{t('impactBody')}</h2></div><button className="circle-arrow" onClick={() => showToast(t('impactDashboardNext'))}><ArrowUpRight size={17} /></button></div><div className="workflow-card"><div className="workflow-header"><div><div className="eyebrow">{t(role)}</div><h2>{t('welcome')}</h2></div><div className="mini-avatar">D</div></div><div className="workflow-steps"><WorkflowStep number="01" text={t('requestStep')} done /><WorkflowStep number="02" text={t('matchStep')} done /><WorkflowStep number="03" text={t('confirmStep')} /></div></div></section><CompletionSummary t={t} /></div> }
...
function ReportsView({ t }: { t: (key: string) => string }) {
  ...
  return <div className="page-wrap inner-page"><div className="page-header"><div><div className="eyebrow"><BarChart3 size={14} /> {t('reports')}</div><h1>{t('reportTitle')}</h1><p>{t('reportBody')}</p></div><span className="report-period">{t('reportPeriod')}</span></div><CompletionSummary t={t} /><div className="panel report-table"><div className="panel-heading"><div><div className="eyebrow">{t('financeSummary')}</div><h2>{t('reportTitle')}</h2></div><span className="status-pill report-status"><span /> {t('matched')}</span></div><div className="report-row report-row-head"><span>{t('workTasks')}</span><span>{t('completedJobs')}</span><span>{t('statusSuccess')}</span></div>{rows.map((row) => <div className="report-row" key={row.label}><strong>{row.label}</strong><span>{row.count}</span><span className="report-match"><Check size={13} /> {row.status}</span></div>)}</div></div>
}
...
function JobsView({ t, language, jobs, query, setQuery, onCreate, onShare, onFindLocation, onOpenMap, showToast }: JobsProps) { return <div className="page-wrap inner-page"><div className="page-header"><div><div className="eyebrow"><ClipboardCheck size={14} /> {t('fieldReady')}</div><h1>{t('browse')}</h1><p>{t('welcomeBody')}</p></div><button className="primary-button" onClick={onCreate}><Plus size={17} /> {t('createJob')}</button></div><div className="toolbar"><div className="search-box"><Search size={17} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder={t('search')} /></div><button className="secondary-button" onClick={() => showToast(t('filtersNext'))}><Settings2 size={16} /> {t('filters')}</button><button className="secondary-button map-toolbar-button" onClick={onFindLocation}><Navigation size={16} /> {t('location')}</button></div><div className="jobs-layout"><div className="panel full-jobs-panel"><div className="jobs-table-head"><span>{t('allJobs')} · {jobs.length}</span><span>{t('updatedNow')}</span></div>{jobs.length > 0 ? jobs.map((job) => <JobRow key={job.id} job={job} language={language} t={t} onClick={() => showToast(`${job.title[language]} ${t('selected')}`)} />) : <div className="empty-state"><Search size={22} /><strong>{t('noJobs')}</strong></div>}</div><div className="panel location-side-panel"><div className="eyebrow">{t('location')}</div><h2>{t('stayClose')}</h2><p>{t('mapsTeamNote')}</p><div className="location-preview"><MapPin size={22} /><span>46.83° N · 106.76° E</span></div><button className="secondary-button full-width" onClick={onShare}><Share2 size={16} /> {t('shareLocation')}</button><button className="text-action" onClick={onOpenMap}>{t('openMaps')} <ExternalLink size={14} /></button></div></div></div> }

function WoolView({ t, onCreate }: { t: (key: string) => string; onCreate: () => void }) { return <div className="page-wrap inner-page"><div className="page-header"><div><div className="eyebrow"><PackageCheck size={14} /> {t('marketAccess')}</div><h1>{t('woolTitle')}</h1><p>{t('woolBody')}</p></div><button className="primary-button" onClick={onCreate}><Plus size={17} /> {t('listWool')}</button></div><div className="wool-hero"><div className="wool-circle"><span>1.86</span><small>{t('tonnes')}</small></div><div><div className="eyebrow">{t('seasonToDate')}</div><h2>{t('qualityMessage')}</h2><p>{t('qualityDescription')}</p><div className="wool-tags"><span>{t('merinoBlend')}</span><span>{t('cleanClip')}</span><span>{t('nearbyBuyers')}</span></div></div></div><div className="market-grid"><MarketItem t={t} titleKey="cleanWhiteWool" locationKey="selenge" amount="420 kg" price="18,000 MNT / kg" /><MarketItem t={t} titleKey="coarseWoolBatch" locationKey="tov" amount="860 kg" price="12,500 MNT / kg" /><MarketItem t={t} titleKey="mixedSpringClip" locationKey="darkhanUul" amount="260 kg" price="15,000 MNT / kg" /></div></div> }
